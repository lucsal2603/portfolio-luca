...
function applyEnglish() {
  ...
  const descs = [
    "A typeface drawn from scratch, letter by letter. The ultimate handmade.",
    "AI consulting for companies. Yes, sometimes the enemy is a client. Pays on time, though.",
    "Helmet e-commerce. Fast as a bike, safe as a helmet.",
    "Formula 1 data and standings, clear even if you only watch the starts.",
    "Landing page for an electric car that doesn't exist. The site works great though.",
    "An art farmhouse in Umbria. Any more handcrafted and it's the Middle Ages.",
    "Restaurant website, made with more care than Sunday tiramisù.",
    "Yes, it says “AI”. The irony did not escape me, don't worry.",
  ];
  document.querySelectorAll(".work__desc").forEach((el, i) => (el.textContent = descs[i] || el.textContent));
  const phs = [
    "( screenshot coming soon )",
    "( handmade image on its way )",
    "( white on purpose, like an art gallery )",
    "( there was an image here, I swear )",
    "( use your imagination )",
    "( screenshot generated by hand )",
  ];
  document.querySelectorAll(".work__ph .mono-label").forEach((el, i) => (el.textContent = phs[i] || el.textContent));
  const cursors = ["see it live →", "see it live →", "wow", "nice, right?", "the details", "handmade", "mamma mia", "the irony"];
  document.querySelectorAll(".work[data-cursor]").forEach((el, i) => el.setAttribute("data-cursor", cursors[i] || "wow"));
  set(".works__count", "TOTAL ( 08 )");
